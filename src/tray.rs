...
use windows_sys::Win32::UI::WindowsAndMessaging::{
    ...
    GetMessageW, GetSystemMetrics, GetWindowLongPtrW, IDI_APPLICATION,
    LR_DEFAULTCOLOR, LookupIconIdFromDirectoryEx, LoadIconW, MF_DISABLED,
    MF_GRAYED, MF_SEPARATOR, MF_STRING, MSG, PostMessageW, PostQuitMessage,
    RegisterClassW, SM_CXSMICON, SM_CYSMICON, SetForegroundWindow,
    SetWindowLongPtrW, TPM_BOTTOMALIGN, TPM_LEFTALIGN, TPM_RIGHTBUTTON,
    TrackPopupMenu, TranslateMessage, WM_APP, WM_COMMAND, WM_DESTROY,
    WM_LBUTTONUP, WM_RBUTTONUP, WNDCLASSW, GWLP_USERDATA, HMENU,
};
...
fn run(
    ...
    unsafe {
        ...
        let h_icon = load_embedded_icon().unwrap_or_else(|| LoadIconW(ptr::null_mut(), IDI_APPLICATION));
...
unsafe fn load_embedded_icon() -> Option<windows_sys::Win32::UI::WindowsAndMessaging::HICON> {
    ...
    if off >= ICON_DATA.len() { return None; }
...
unsafe extern "system" fn wnd_proc(
    ...
    match msg {
        ...
        WM_COMMAND => {
            ...
            let state = &*state_ptr;
            match id {
                ID_OPEN_DASH => { let _ = open_url(&state.web_url); }
                ID_OPEN_DOCK => { let _ = open_url(&state.dock_url); }
                ID_CUT_DELAY => {
...
unsafe fn show_menu(hwnd: HWND) {
    ...
    let cut_flags = if state.ctrl.phase() == "active" { MF_STRING } else { MF_STRING | MF_DISABLED | MF_GRAYED };
...
fn status_label(ctrl: &Controller) -> String {
    ...
    match phase {
        ...
        _ => {
            if ctrl.ingest_alive() { "Status: LIVE (no delay)".into() }
            else { "Status: idle (no source)".into() }
        }
    }
}
...
fn obs_rtmp_url(s: &Settings) -> String {
    let host = if s.ingest_bind_all { "0.0.0.0" } else { "127.0.0.1" };
